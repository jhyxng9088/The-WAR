...
export class RenderPerformance {
  ...
  private adjustPixelRatio(): void {
    ...
    // Reallocating the drawing buffer while the map is moving causes a visible
    // hitch on iPad. Start sharp at 1.5x and only step down after sustained load;
    // never bounce the resolution back up during the same session.
