...
function collectTrees(): TreePoint[] {
  ...
  for (let x = -WORLD_HALF_WIDTH + 4; x <= WORLD_HALF_WIDTH - 4; x += spacing) {
    for (let z = -WORLD_HALF_DEPTH + 4; z <= WORLD_HALF_DEPTH - 4; z += spacing) {
      ...
      const chance = THREE.MathUtils.clamp((density - 0.34) * 1.22, 0, 0.74);
      if (density < 0.42 || deterministic01(px, pz, 29) > chance) continue;
      ...
      points.push({
        ...
        z: pz,
        scale: (1.05 + deterministic01(px, pz, 31) * 0.58) * uplandScale,
        rotation: deterministic01(px, pz, 37) * Math.PI * 2,
        variant: mountain > 0.4 || deterministic01(px, pz, 41) > 0.72 ? 1 : 0,
...
function addTreeVariant(scene: THREE.Scene, points: readonly TreePoint[], variant: 0 | 1): void {
  const geometry = variant === 0
    ...
    : new THREE.ConeGeometry(0.34, 1.04, 7);

  if (variant === 0) {
    geometry.scale(1.16, 1.02, 1.16);
  ...
  const dark = new THREE.Color(variant === 0 ? 0x31583a : 0x294c38);
  const light = new THREE.Color(variant === 0 ? 0x587a4c : 0x47694b);
